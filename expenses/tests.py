...
class ExpenseInvitationTests(TestCase):
    def setUp(self):
        User = get_user_model()
        self.creator = User.objects.create_user(username='creator', email='creator@gmail.com', password='pass12345')
        self.invited = User.objects.create_user(username='invited', email='invited@gmail.com', password='pass12345')
    ...
    def test_creator_can_email_invitation_and_acceptance_recalculates_split(self):
        ...
        response = self.client.post(reverse('invite_to_expense', args=[self.expense.pk]), {'email': 'invited@gmail.com'})
        self.assertEqual(response.status_code, 302)
