...
def invite_to_expense(request, pk):
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        invitation = ExpenseInvitation.objects.filter(expense=expense, email=email, status=ExpenseInvitation.Status.PENDING).first()
        created = invitation is None
        if created:
            invitation = ExpenseInvitation(expense=expense, email=email, invited_by=request.user)
        if not created:
